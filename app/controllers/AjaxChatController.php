...
class AjaxChatController extends BaseController
{
    ...
    public function getMessages()
    {
        ...
        $chats = $this->messages($conversationId, $lastId);
    ...
    public function getLatestConversation()
    {
        ...
        $latestConversation = Conversation::where('group_id', '=', $groupId)
            ->orderBy('conversation_id', 'DESC')
            ->first();
        if(!empty($latestConversation)) {
            // call the getMessage function
            return View::make('ajax.chat.messages')
                ->with('chats', $this->messages($latestConversation->conversation_id));
        }
    }

    public function getArchivedConversation()
    {
        $groupId = Input::get('group_id');
        $conversationId = Input::get('conversation_id');

        return View::make('ajax.chat.messages')
            ->with('chats', $this->messages($conversationId));
    }

    protected function messages($conversationId, $lastId = null)
    {
        if(isset($lastId)) {
            $chats = ChatConversation::where('conversation_id', '=', $conversationId)
